...
<title><%=newJcpeizhi.getShujuBieming()%><%=newJcpeizhi.getSjshaochuBieming()%>管理</title>
...
var yonghuId = <%=yonghuId%>;
	...
	function openSjshaochuModifyDialog(){
		...
		$("#fm").form("load",row);
		url="../addSjpinglun?shujuId="+row.shujuId+"&yonghuId="+row.yonghuId;
	}
	...
	function quxiaoSjshaochu(){
		...
		$.messager.confirm("系统提示","您确认要执行吗？",function(r){
			if(r){
				$.post(url,{sjshaochuType:6},function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						$("#dg").datagrid("reload");
					}else{
	...
	function fukuanSjshaochu(){
		...
		$.messager.confirm("系统提示","您确认要处理这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../fukuanSjshaochu",{delIds:ids,sjshaochuType:1,yonghuId:yonghuId},function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						$("#dg").datagrid("reload");
					}else{
...
<!--startprint-->
	<table id="dg" title="<%=newJcpeizhi.getShujuBieming()%><%=newJcpeizhi.getSjshaochuBieming()%>" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getSjshaochus?sjshaochuType=0&yonghuId=<%=yonghuId %>" fit="true" rownumbers="true" toolbar="#tb">
		<thead>
			<tr>
				...
				<th field="sjshaochuDouble" width="10">元/月</th>
				...
				<th field="sjshaochuMark2" width="40">电话</th>
				<th field="sjshaochuMark3" width="60">身份证</th>
	...
	<div id="tb">
		<div>
			<a href="javascript:shenheSjshaochu()" class="easyui-linkbutton" iconCls="icon-add" plain="true">租赁</a>
			<a href="javascript:deleteSjshaochu()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除</a>
			<a href="javascript:openSjshaochuModifyDialog()" class="easyui-linkbutton" iconCls="icon-add" plain="true">交流</a>
		</div>
		<div>
